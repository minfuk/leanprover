...
(η : Π(a : C), @hom _ D^.struct (F a) (G a))
(commute : Π {a b : C} (f : @hom _ C^.struct a b),
  @comp _ D^.struct _ _ _ (G^.morphism f) (η a) = @comp _ D^.struct _ _ _ (η b) (F^.morphism f))
...
namespace natural_transformation
  ...
  let infixr `⟶` := λ(a b : D), @hom _ D^.struct a b in
  let infixr `∘` := λ{a b c : D} (f : b ⟶ c) (g : a ⟶ b), @comp _ D^.struct _ _ _ f g in
  have H : (η₃ ∘n (η₂ ∘n η₁))^.η = ((η₃ ∘n η₂) ∘n η₁)^.η, from
  ...
  have H : (natural_transformation.compose natural_transformation.id η₁)^.η = η₁^.η, from
  ...
  have H : (natural_transformation.compose η₁ natural_transformation.id)^.η = η₁^.η, from
